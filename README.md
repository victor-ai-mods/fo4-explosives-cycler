...
MCM hotkeys - next / previous list, next / previous item in the current list, group 1 / 2 / 3 -
or the reusable "Explosives Cycler" Aid item switch between them round-robin and show
...
weak mines, strong mines, Molotov cocktail.

Groups (`"group": "1"` or `"1,3"` in a list): a group hotkey switches between the lists of its
group; pressed while a list of another group is current, it returns to the list and item last
used in its group. Defaults: group 1 = grenades + Molotov, group 2 = mines, group 3 = Molotov. Edit them in
